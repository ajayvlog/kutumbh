        ...
        <main>
            <div class="container-fluid">
                ...
                <div class="card mb-4">
                    ...
                    @endif
                    <div class="card-body">
                        <form action="{{ route('admin.tehsils.update',$tehsil->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                ...
                                <select name="state_id" class="form-control" style="width:500px">
                                    ...
                                    @foreach ($states as $key => $value)
                                        <option value="{{ $key }}" {{ $key == $tehsil->state_id ? 'selected':'' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                @foreach ($cities as $key => $value)
                                    <option value="{{ $key }}" {{ $key == $tehsil->city_id ? 'selected':'' }}>{{ $value }}</option>
                                @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <strong>Tehsil Name:</strong>
                                <input type="text" name="tehsil_name" value="{{ $tehsil->tehsil_name }}" class="form-control" placeholder="Enter Tehsil Name" style="width:500px">
                            </div>
